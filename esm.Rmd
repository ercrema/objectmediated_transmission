...
2. *Encoding Error* With probability $\mu_{e}$, each object value is associated with a new integer value that has not been introduced in the simulation run. 
...
4. *Decoding Error* With probability $\mu_{d}$, each agent updates its mental template value with a new number that has not been previously used in the simulation run. This portrays an instance where the individual fails to correctly reconstruct the mental template of an object.
...
In experiment 1 we ran our simulations using different combinations of population size $N$ and production rate $\lambda$ for each of the two error type (i.e., encoding and decoding), and recorded the richness $k$ (the number of unique variants) and diversity $D$ (Simpson's Index, equivalent to $1-\sum_{i}^{k}p_{i}^2$, with $p_i$ being the proportion of the $i$-th variant) for the population of objects and mental templates. We used the following parameter settings, recording the two summary statistics after 5,000 time-steps.
...
In most real-world situations we do not have estimates of $N$ or $\lambda$, and we simply infer modes of tranmission from the frequency distribution of the observed objects. In experiment 2 we emulate this condition by exploring four different combinations of $\lambda$ and $N$, ensuring that the number of objects produced at each time step was, on average, equal to 1,000. We compare the output of these settings against a conventional Wright-Fisher Infinite Allele model with a population size of 1,000 to determine the consequence of implicitely assuming the population size to be comparable to the the number of objects. Detailed parameter settings are as follows:
...
text(5,0.2,"Encoding Error",srt=90,cex=0.8)
